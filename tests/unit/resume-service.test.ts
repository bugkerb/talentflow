...
vi.mock("pdf-parse", () => ({
  PDFParse: class {
    async getText() { return { text: "PDF candidate experience" }; }
    async destroy() {}
  }
}));

vi.mock("mammoth", () => ({
  extractRawText: async () => ({ value: "DOCX candidate experience" })
}));
...
describe("ResumeService", () => {
  ...
  it("extracts text from validated PDF resumes and always destroys the parser", async () => { await expect(service.extractText(pdf())).resolves.toBe("PDF candidate experience"); });
  it("extracts text from validated DOCX resumes", async () => { await expect(service.extractText(pdf("resume.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", new Uint8Array([0x50, 0x4b, 0x03, 0x04])))).resolves.toBe("DOCX candidate experience"); });
