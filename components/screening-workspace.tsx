...
export function ScreeningWorkspace({ data }: { data: ScreeningWorkspaceData }) {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      const extractedText = resumeText.trim() || (file ? await file.text() : "");
      if (!extractedText.trim()) { setError("ไม่พบข้อความในเรซูเม่ กรุณาวางข้อความเรซูเม่ก่อนเริ่มการวิเคราะห์"); return; }
      const response = await runScreening({ applicationId: target.applicationId, resumeId: selectedResumeId, jobDescription: target.jobDescription, resumeText: extractedText });
...
<strong className="mt-2 block font-serif text-4xl text-[#004cca]">{result.score}<span className="text-base text-[#565e74]">/100</span>
</strong>
</div>
