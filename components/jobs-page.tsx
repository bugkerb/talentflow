...
import { closeJob, createJob, pauseJob, publishJob, resumeJob, updateJob } from "../app/jobs/actions";
...
export function JobsPage({ initialJobs }: { initialJobs: JobRecord[] }) {
  ...
  function submitDraft(formData: FormData) {
    startTransition(async () => {
      const result = await createJob({ title: formData.get("title"), description: formData.get("description"), department: formData.get("department") || undefined, status: formData.get("status") });
  ...
  function resume(job: JobRecord) { startTransition(async () => { const result = await resumeJob(job.id, job.version); if (result.error || !result.data) { setMessage(result.error?.message ?? "ดำเนินการไม่สำเร็จ"); return; } replaceJob(result.data as JobRecord); setMessage("เปิดรับสมัครอีกครั้งแล้ว"); }); }
  ...
  return <AppShell><Sidebar activePath="/jobs" /><Header activePath="/jobs" /><main className="min-h-[calc(100vh-4rem)] bg-[#f7f9fb] px-4 py-6 md:ml-[260px] md:px-8 lg:px-10">
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <section className="space-y-4 xl:col-span-8"><div className="mb-2 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#e0e3e5] bg-white p-4 shadow-sm"><div className="flex flex-wrap gap-2" role="group" aria-label="กรองตำแหน่งงานตามสถานะ">
        {([['all', `ทั้งหมด (${jobs.length})`], ['open', 'เปิดรับ'], ['draft', 'ฉบับร่าง'], ['paused', 'หยุดชั่วคราว'], ['closed', 'ปิดรับ']] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setStatusFilter(value)} aria-pressed={statusFilter === value} className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${statusFilter === value ? 'bg-[#0062ff] text-white' : 'bg-[#eceef0] text-[#424656] hover:bg-[#e0e3e5]'}`}>{label}</button>)}
      </div></div>
