...
export function JobsPage({ initialJobs }: { initialJobs: JobRecord[] }) {
  ...
  return <AppShell><Sidebar activePath="/jobs" /><Header activePath="/jobs" /><main className="min-h-[calc(100vh-4rem)] bg-[#f7f9fb] px-4 py-6 md:ml-[260px] md:px-8 lg:px-10">
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      </div><button type="button" onClick={() => setFilterOpen((open) => !open)} aria-expanded={filterOpen} aria-controls="job-filter-panel" className="flex items-center gap-1 text-sm text-[#565e74] transition-colors hover:text-[#004cca]"><span aria-hidden="true" className="material-symbols-outlined text-base">filter_list</span>ตัวกรอง</button></div>{filterOpen && <div id="job-filter-panel" className="rounded-xl border border-[#e0e3e5] bg-white p-4 shadow-sm" role="region" aria-label="ตัวกรองเพิ่มเติม"><div className="flex items-center justify-between"><p className="text-sm font-semibold">กรองตามสถานะ</p><button type="button" onClick={() => setStatusFilter("all")} className="text-xs font-semibold text-[#004cca]">ล้างตัวกรอง</button></div><div className="mt-3 flex flex-wrap gap-2">{(Object.entries(statusLabels) as Array<[JobRecord["status"], string]>).map(([value, label]) => <button key={value} type="button" onClick={() => setStatusFilter(value)} aria-pressed={statusFilter === value} className={`rounded-full px-3 py-1.5 text-xs font-semibold ${statusFilter === value ? "bg-[#0062ff] text-white" : "bg-[#eceef0] text-[#424656]"}`}>{label}</button>)}</div></div>}
