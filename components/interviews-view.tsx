...
const createInterviewEvents = (today: Date): InterviewEvent[] => {
  const todayKey = dateKey(today);
  return [
    { id: "ux-research", date: dateKey(addDays(startOfWeek(today), 1)), title: "นักวิจัยผู้ใช้", candidate: "สมหญิง ใจดี", kind: "scheduled", top: 8, height: 60 },
    { id: "senior-developer", date: todayKey, title: "นักพัฒนาอาวุโส", candidate: "กฤษณะ แสงอรุณ", kind: "conflict", top: 136, height: 60 },
    { id: "team-sync", date: todayKey, title: "ประชุมทีม", candidate: "ภายในทีม", detail: "นัดหมายซ้อนทับ", kind: "internal", top: 168, height: 60 },
    { id: "product-manager", date: dateKey(addDays(today, 2)), title: "ผู้จัดการผลิตภัณฑ์ · รอบสุดท้าย", candidate: "วิชัย รักษา", detail: "ออนไลน์ · Google Meet", kind: "scheduled", top: 264, height: 120 },
  ];
};
...
export function InterviewsView() {
  ...
  const interviewEvents = useMemo(() => createInterviewEvents(today), [today]);
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setMessage("บันทึกการเลื่อนนัดหมายแล้ว"); };
