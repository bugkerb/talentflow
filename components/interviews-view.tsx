...
import type { CalendarEventSummary, InterviewListItem } from "@/application/interview-ports";
...
function CalendarEventsPanel({ events, error }: { events: CalendarEventSummary[]; error: string }) {
  if (error) return <section role="status" className="mx-auto mb-6 max-w-7xl rounded-xl border border-[#ba1a1a]/30 bg-[#fff7f5] p-4 text-sm text-[#93000a] md:ml-[260px]">ยังโหลดกิจกรรมจาก Google Calendar ไม่ได้ — {error.includes("not connected") || error.includes("not configured") ? "เชื่อมต่อ Google Calendar ในตั้งค่าก่อน" : "ลองใหม่ภายหลัง"}</section>;
...
export function InterviewsView({ initialInterviews = [], initialCalendarEvents = [], calendarError = "", initialScheduleApplicationId = "", initialScheduleDate = "", initialScheduleStart = "", initialScheduleEnd = "" }: { initialInterviews?: InterviewListItem[]; initialCalendarEvents?: CalendarEventSummary[]; calendarError?: string; initialScheduleApplicationId?: string; initialScheduleDate?: string; initialScheduleStart?: string; initialScheduleEnd?: string }) {
  ...
  return <AppShell><Sidebar activePath="/interviews" /><Header /><CalendarEventsPanel events={initialCalendarEvents} error={calendarError} /><main className="min-h-screen bg-[#f8f9fb] md:ml-[260px]"><div className="px-4 py-5 sm:px-6 sm:py-6">
