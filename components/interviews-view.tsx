...
const toEvent = (interview: InterviewListItem): InterviewEvent => {
  ...
  return { id: interview.id, date: dateKey(start), title: interview.interviewType, candidate: interview.candidateName, detail: `${interview.jobTitle} · ${interview.status === "cancelled" ? "ไม่พร้อมใช้งาน" : interview.googleMeetUrl ? "ออนไลน์" : "นัดหมาย"}`, kind: interview.status === "cancelled" ? "internal" : "scheduled", top: Math.max(0, (start.getHours() - 9) * 64 + start.getMinutes()), height: Math.max(48, duration), interview };
};
...
function AppointmentModal({ interview, rescheduleMode, date, onDateChange, onClose, onReschedule, onCancel, onSubmit, timeRef, message }: { interview: InterviewListItem; rescheduleMode: boolean; date: string; onDateChange: (value: string) => void; onClose: () => void; onReschedule: () => void; onCancel: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; timeRef: React.Ref<HTMLInputElement>; message: string }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#071d37]/45 p-4"><section role="dialog" aria-modal="true" aria-labelledby="appointment-modal-heading" className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-[#0057d9]">รายละเอียดนัดหมาย</p><h2 id="appointment-modal-heading" className="mt-1 text-xl font-bold">{interview.candidateName}</h2><p className="mt-1 text-sm text-[#565e74]">{interview.interviewType} · {interview.jobTitle}</p></div><button type="button" aria-label="ปิดรายละเอียดนัดหมาย" onClick={onClose} className="rounded-lg p-1 text-[#565e74] hover:bg-[#f2f4f6]"><span className="material-symbols-outlined">close</span></button></div><div className="mt-5 grid gap-3 rounded-xl bg-[#f8f9fb] p-4 text-sm"><p><strong>ผู้สมัคร:</strong> {interview.candidateName}</p><p><strong>ผู้สัมภาษณ์:</strong> {interview.interviewerName}</p><p><strong>วันเวลา:</strong> {new Intl.DateTimeFormat("th-TH", { dateStyle: "full", timeStyle: "short" }).format(new Date(interview.startsAt))}</p><p><strong>รูปแบบ:</strong> {interview.format === "onsite" ? "ที่สำนักงาน" : "ออนไลน์ (Google Meet)"}</p>{interview.googleMeetUrl && <a className="font-semibold text-[#0057d9] underline" href={interview.googleMeetUrl} target="_blank" rel="noreferrer">เปิด Google Meet</a>}</div>{rescheduleMode && <form onSubmit={onSubmit} className="mt-5 space-y-4"><label className="block text-xs font-bold text-[#565e74]">วันที่<input name="date" type="date" value={date} onChange={(event) => onDateChange(event.target.value)} className="mt-1 w-full rounded-lg border border-[#c2c6d9] px-3 py-2 text-sm" /></label><label className="block text-xs font-bold text-[#565e74]">เวลาเริ่มต้น<input ref={timeRef} name="time" type="time" defaultValue={new Date(interview.startsAt).toTimeString().slice(0, 5)} className="mt-1 w-full rounded-lg border border-[#c2c6d9] px-3 py-2 text-sm" /></label><label className="block text-xs font-bold text-[#565e74]">เหตุผลการเปลี่ยนแปลง<textarea name="reason" className="h-20 w-full rounded-lg border border-[#c2c6d9] p-3 text-sm" placeholder="ระบุเหตุผล (ถ้ามี)" /></label><button type="submit" className="w-full rounded-lg bg-[#071d37] py-2.5 text-sm font-semibold text-white">บันทึกการเลื่อนนัด</button></form>}<div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={onReschedule} disabled={interview.status === "cancelled"} className="rounded-lg bg-[#071d37] px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">เลื่อนนัด</button><button type="button" onClick={onCancel} disabled={interview.status === "cancelled"} className="rounded-lg border border-[#ba1a1a] px-4 py-2.5 text-sm font-semibold text-[#ba1a1a] disabled:cursor-not-allowed disabled:opacity-50">ยกเลิกนัด</button><button type="button" onClick={onClose} className="rounded-lg border border-[#c2c6d9] px-4 py-2.5 text-sm">ปิด</button></div>{message && <p role="status" className="mt-3 rounded-lg bg-[#dbe3ff] p-3 text-sm">{message}</p>}</section></div>;
}
...
export function InterviewsView({ initialInterviews = [], initialScheduleApplicationId = "", initialScheduleDate = "", initialScheduleStart = "", initialScheduleEnd = "" }: { initialInterviews?: InterviewListItem[]; initialScheduleApplicationId?: string; initialScheduleDate?: string; initialScheduleStart?: string; initialScheduleEnd?: string }) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const target = interviews.find((item) => item.id === detailTargetId); if (!target) { setMessage("คลิกนัดหมายบนปฏิทินเพื่อเลือกก่อนแก้ไข"); return; } if (target.status === "cancelled") { setMessage("นัดหมายที่ยกเลิกแล้วไม่สามารถเลื่อนนัดได้"); return; } const start = new Date(`${form.get("date")}T${form.get("time")}:00`); const result = await rescheduleInterview({ interviewId: target.id, startsAt: start.toISOString(), endsAt: new Date(start.getTime() + 30 * 60000).toISOString(), reason: String(form.get("reason") ?? "") }, target.version); if ("data" in result && result.data) { setInterviews((current) => current.map((item) => item.id === target.id ? { ...item, ...result.data } : item)); setMessage("บันทึกการเลื่อนนัดหมายแล้ว"); } else setMessage(actionError(result)); };
  ...
  const selectEvent = (event: InterviewEvent): void => { setDetailTargetId(event.interview.id); setDetailDate(formatInputDate(new Date(event.interview.startsAt))); setRescheduleMode(false); setDetailOpen(true); };
  useEffect(() => {
