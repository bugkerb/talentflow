...
export class SupabaseApplicationRepository implements ApplicationRepository {
  ...
  async updateStage(id: string, expectedVersion: number, stage: Application["stage"], actorId: string) { const { data, error } = await this.client.from("applications").update({ stage, version: expectedVersion + 1, updated_by: actorId, stage_changed_at: new Date().toISOString() }).eq("id", id).eq("version", expectedVersion).is("deleted_at", null).select(columns).maybeSingle(); throwOnError(error); return data ? toRecord(data as ApplicationRow) : null; }
