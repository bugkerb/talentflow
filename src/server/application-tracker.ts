...
export const loadApplicationTracker = async (): Promise<ApplicationTrackerData> => {
  ...
  const [applicationsResult, candidatesResult, jobsResult] = await Promise.all([
    supabase.from("applications").select("id,candidate_id,job_id,stage,status,version,applied_at,candidates!inner(id,full_name,email,source),jobs!inner(id,title,status)").eq("status", "active").is("deleted_at", null).order("updated_at", { ascending: false }),
    supabase.from("candidates").select("id,full_name,email,source").is("deleted_at", null).order("full_name"),
  ...
  const candidates = (candidatesResult.data ?? []).map((row) => ({ id: row.id, fullName: row.full_name, email: row.email, source: row.source }));
