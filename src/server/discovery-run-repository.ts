...
export class SupabaseDiscoveryRunRepository implements DiscoveryRepository {
  ...
  async saveResults(runId: string, results: RankedDiscoveryCandidate[], actorId: string) {
    if (results.length) {
      const candidates = results.map((result) => ({ full_name: result.fullName, email: result.email ?? null, phone: result.phone ?? null, profile_url: result.profileUrl, source: "discovery" as const, source_detail: `${result.source}:${result.externalId}`, discovery_identity: `${result.source}:${result.externalId}`, normalized_profile: result.normalizedProfile, data_status: "unverified", data_origin: result.source === "demo" ? "fixture" : "live", created_by: actorId, updated_by: actorId }));
      const { data: candidateRows, error: candidateError } = await this.client.from("candidates").upsert(candidates, { onConflict: "source,discovery_identity" }).select("id,source_detail,discovery_identity");
