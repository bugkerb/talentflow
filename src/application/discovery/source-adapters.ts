...
export const createConfiguredDiscoveryAdapter = (): DiscoverySourceAdapter => {
  const endpoint = process.env.DISCOVERY_SOURCE_ENDPOINT?.trim();
  const apiKey = process.env.DISCOVERY_SOURCE_API_KEY?.trim();
  if (!endpoint || !apiKey) return { search: async () => { throw new AppError("DATABASE_ERROR", "ยังไม่ได้ตั้งค่าแหล่งข้อมูลภายนอกสำหรับการค้นหาผู้สมัคร"); } };
