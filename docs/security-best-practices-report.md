...
| RLS integration | FAIL | `npm run test:integration`: sandbox run failed before reset; permitted rerun produced no SQL assertions and was terminated after stalling, exit 130 | `rls_active_hr_allowed`, `rls_viewer_denied`, and `rls_inactive_hr_denied` at `supabase/verify.sql:76-115` are **NOT RUN**. |
| Auth/browser E2E | FAIL | `npm run test:e2e`, exit 1: `local Supabase status timed out after 30 seconds` | Assertions in `tests/e2e/auth.spec.ts:4-48` are **NOT RUN**. No runtime proof exists for anonymous denial, cookie refresh, login, or logout. |
| Production dependency audit | NOT RUN | `npm audit --omit=dev --audit-level=high` could not reach the npm advisory endpoint; escalation was rejected because it would disclose the dependency tree to the public registry | No claim that high/critical advisories are absent. |
...
|---|---|---|
| A01 Broken Access Control | FAIL | Current-worktree remediation exists, but RLS and auth E2E assertions are NOT RUN; see TF-SEC-001. Future object-level authorization/IDOR tests belong to [#3](https://github.com/bugkerb/talentflow/issues/3), [#4](https://github.com/bugkerb/talentflow/issues/4), [#5](https://github.com/bugkerb/talentflow/issues/5), [#7](https://github.com/bugkerb/talentflow/issues/7), and [#8](https://github.com/bugkerb/talentflow/issues/8). |
...
| A06 Vulnerable and Outdated Components | NOT RUN | Advisory audit did not execute; see TF-SEC-004. |
| A07 Identification and Authentication Failures | FAIL | Generic login errors and server `getUser()` validation are present, but throttling is absent and browser session/cookie assertions are NOT RUN; see TF-SEC-001 and TF-SEC-002. |
